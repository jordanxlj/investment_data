...
try:
    ...
    try:
        ...
        evaluate_brokerage_report = eval_module.evaluate_brokerage_report
...
def test_weighted_median_mismatched_lengths():
    ...
    with pytest.raises(ValueError, match="Values and weights must have the same length"):
        weighted_median(np.array([1]), np.array([1, 2]))
...
def test_aggregate_forecasts_empty_fields():
    ...
    result = aggregate_forecasts(test_df, 'bullish')
...
def test_get_brokerage_consensus_sentiment_tie(mock_engine):
    ...
    df = pd.DataFrame({
        ...
        'rating': ['买入', '中性'],
        'report_type': ['点评', '点评'],
        'rating_category': ['BUY', 'NEUTRAL']  # Pre-set to avoid apply
    })
    with patch('pandas.read_sql') as mock_read_sql, patch('evaluate_brokerage_report.classify_rating') as mock_classify:
        mock_read_sql.return_value = df
...
def test_get_next_year_consensus_pattern(month, expected_pattern, mock_engine):
    ...
    with patch('pandas.read_sql') as mock_read_sql:
        ...
        get_next_year_consensus(mock_engine, '000001.SZ', eval_date, '2025')
...
def test_get_next_year_consensus_weight_error(mock_engine, caplog):
    ...
    with patch('pandas.read_sql') as mock_read_sql, patch('evaluate_brokerage_report.get_report_weight') as mock_weight:
        mock_read_sql.return_value = df
        mock_weight.side_effect = Exception("weight error")
        result = get_next_year_consensus(mock_engine, '000001.SZ', '20250101', '2025')
        assert result is not None
        assert "Error applying report weights" in caplog.text
        assert result['avg_report_weight'] == DEFAULT_REPORT_WEIGHT

def test_get_next_year_consensus_agg_error(mock_engine, caplog):
    """Test error in aggregate_forecasts"""
    df = pd.DataFrame({'report_type': ['点评']})
    with patch('pandas.read_sql') as mock_read_sql, patch('evaluate_brokerage_report.aggregate_forecasts') as mock_agg:
        mock_read_sql.return_value = df
...
def test_get_annual_report_data(mock_engine, df_empty, fund_empty):
    ...
    with patch('pandas.read_sql') as mock_read_sql:
        ...
        result = get_annual_report_data(mock_engine, '000001.SZ', '20250101', '20241231')
        if df_empty:
            assert result is None
        else:
...
def test_process_stock_consensus_no_data(mock_engine):
    ...
    with patch('evaluate_brokerage_report.get_brokerage_consensus') as mock_cons, \
         patch('evaluate_brokerage_report.get_annual_report_data') as mock_annual, \
         patch('evaluate_brokerage_report.get_next_year_consensus') as mock_next:
        mock_cons.return_value = None
        mock_annual.return_value = None
        mock_next.return_value = None
        result = process_stock_consensus(mock_engine, '000001.SZ', '20250101')
        assert result is None
...
def test_process_stock_consensus_with_next_year(mock_engine):
    ...
    with patch('evaluate_brokerage_report.get_brokerage_consensus') as mock_cons, \
        ...
        mock_next.return_value = {'eps': 2.0, 'total_reports': 5, 'avg_report_weight': 3.0}
        result = process_stock_consensus(mock_engine, '000001.SZ', '20250101')
...
def test_upsert_batch_empty_df(mock_engine):
    ...
    result = _upsert_batch(mock_engine, pd.DataFrame())
    assert result == 0
...
def test_upsert_batch(mock_engine):
    ...
    df = pd.DataFrame({
        ...
        # Add other columns as None or values
    }).reindex(columns=ALL_COLUMNS).fillna(None)
    with patch('sqlalchemy.MetaData') as mock_meta, \
        ...
        result = _upsert_batch(mock_engine, df)
        assert result == 1
...
def test_get_stocks_list_specific(mock_engine, stocks_input, expected):
    ...
    result = get_stocks_list(mock_engine, stocks_input)
...
def test_get_stocks_list_query(mock_engine):
    ...
    result = get_stocks_list(mock_engine)
...
def test_get_report_weight_error_conversion(caplog):
    ...
    result = get_report_weight(BadType())
    assert result == DEFAULT_REPORT_WEIGHT
...
def test_get_trade_cal_error():
    ...
    with patch('evaluate_brokerage_report.pro.trade_cal') as mock_cal:
        mock_cal.side_effect = Exception("API error")
        result = get_trade_cal('20250101', '20250101')
        assert result.empty
